...
class SelectWitnessView extends StatelessWidget
    implements SelectWitnessViewContract {
  const SelectWitnessView({super.key, required this.controller});
  final SelectWitnessControllerContract controller;
  @override
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          Container(
            ...
            child: Column(
              children: [
                24.h.heightBox,
                AppText(
                  "Selected witness will be notified to accept or decline your request",
                  fontSize: 14,
      ...
      bottomNavigationBar: MultiBlocListener(
        listeners: [
          BlocListener<CreateInvestmentCubit, CreateInvestmentState>(
            listener: (context, state) {
              state.whenOrNull(
                loading: () => AppDialog.showAppProgressDialog(context),
                success: (response) {
                  context.pop();
                  GetIt.I.get<IWidgetHelper>().showSuccessToast(context,
        ...
        child: CustomBottomButtonWrapperWidget(
          ...
          onPressed: controller.selectedReferees.isEmpty
              ? null
              : () => AppSheets.investmentConfirmationSheet(context,
                      isNormalLoan: false,
                      applyForLoan: () => controller.onRequestInvesstment(),
                      loanInfo: {
                        "Loan type": (controller.data['investment_type_id']
                                as InvestmentTypeData)
                            .name,
                        "Requested amount":
                            controller.data['amount'].toString(),
                        "Seller/Vendor name": controller.data['vendor_name'],
                        "Seller/Vendor contact":
                            controller.data['vendor_contact'],
                        for (var referee = 0;
                            referee <
                                (controller.data['referee_ids'] as List).length;
                            referee++)
                          "Referees ${referee + 1}":
                              "${((controller.data['referee_ids'] as List)[referee] as RefereeData).firstName} ${((controller.data['referee_ids'] as List)[referee] as RefereeData).lastName}",
                        for (var referee = 0;
